...
add_action('woocommerce_add_to_cart', 'cart_modify_dimensions');

function cart_modify_dimensions($cart_item_key) {
	
	$cart = WC()->cart->get_cart();
	foreach($cart as $cartItemKey=>$cartItem) {
		if ($cartItemKey == $cart_item_key) {
			if (array_key_exists('_gravity_form_data', $cartItem)) {
				$formData = $cartItem['_gravity_form_data'];
				$formId = $formData['id'];
				...
				if (!empty($form)) {
					...
					foreach($formFields as $fieldKey=>$fieldData) {
						...
						$fieldKeys[$fieldId] = array(
							'label'=>$fieldData->label,
							'adminLabel'=>$fieldData->adminLabel,
							'input-name'=>$fieldData->inputName,
						);
						
						if (is_array($fieldData['inputs'])) {
							foreach($fieldData['inputs'] as $inputId=>$inputObj) {
								$newFieldId = $fieldId.'.'.$inputId;
								$fieldKeys[$newFieldId] = array(
				...
				$formDataFinal = array();
				$formInfo = $cartItem['_gravity_form_lead'];
				foreach($formInfo as $infoId=>$infoValue) {
					if (array_key_exists($infoId, $fieldKeys)) {
						$formItem = $fieldKeys[$infoId];
						if (!empty($formItem['adminLabel'])){
							$formItemLabel = $formItem['adminLabel'];
						} else if (!empty($formItem['input-name'])) {
							$formItemLabel = $formItem['input-name'];
						} else {
							$formItemLabel = sanitize_title($formItem['label']);
						}
						$formDataFinal[$formItemLabel] = $infoValue;
					}
				}
				
				if ($formDataFinal['use-shipping-calculator'] == 1) {
					//$formInfo = $cartItem['_gravity_form_lead'];
					$bannerHeightFt = $formDataFinal['height-ft'];
					$bannerHeightIn = $formDataFinal['height-in'];
					$bannerHeight = ($bannerHeightFt * 12) + $bannerHeightIn;
					
					$bannerWidthFt = $formDataFinal['width-ft'];
					$bannerWidthIn = $formDataFinal['width-in'];
					$bannerWidth = ($bannerWidthFt * 12) + $bannerWidthIn;
					
					/* Calculation to determine which side is the ship height */
					if ($bannerWidth > $bannerHeight) {
						$bannerShipHeight = $bannerWidth;
					} else {
						$bannerShipHeight = $bannerHeight;
					}
					
					$formDataFinal['shipping-height'] = $bannerShipHeight;
					//$bannerHeight and $bannerWidth are now in Inches.
					//Add the dimensions to the product.
					$bannerWeight = $formDataFinal['_weight'];
					$bannerShipDepth = $formDataFinal['shipping-depth'];
					$bannerShipWidth = $formDataFinal['shipping-width'];
					
					if ($bannerShipDepth < 1) { $bannerShipDepth = 1; }	//Minimum 1-inch length.
					if ($bannerShipHeight < 1) { $bannerShipHeight = 1; }	//Minimum 1-inch length.
					if ($bannerShipWidth < 1) { $bannerShipWidth = 1; }		//Minimum 1-inch length.
					if ($bannerWeight < 1) { $bannerWeight = 1; }	//Minimum 1-inch length.
					
					//Add Custom Shipping Fields.
					WC()->cart->cart_contents[$cartItemKey]['shipping']['height'] = $bannerShipHeight;
					WC()->cart->cart_contents[$cartItemKey]['shipping']['width'] = $bannerShipWidth;
					WC()->cart->cart_contents[$cartItemKey]['shipping']['length'] = $bannerShipDepth;
					WC()->cart->cart_contents[$cartItemKey]['shipping']['weight'] = $bannerWeight;
				}
			}
...
function filter_woocommerce_cart_shipping_packages( $packages ) { 
	
	if (is_array($packages)) {
		foreach($packages as $packageKey=>$package) {
			if (array_key_exists('contents', $package)) {
				$packageContents = $package['contents'];
				if (is_array($packageContents)) {
					foreach($packageContents as $cartKey=>$cartItem) {
						if (array_key_exists('shipping', $cartItem)) {
							//If we have our custom shipping fields (as determined from above), set the product.
							$packages[$packageKey]['contents'][$cartKey]['data']->width = $cartItem['shipping']['width'];
							$packages[$packageKey]['contents'][$cartKey]['data']->length = $cartItem['shipping']['length'];
							$packages[$packageKey]['contents'][$cartKey]['data']->height = $cartItem['shipping']['height'];
							$packages[$packageKey]['contents'][$cartKey]['data']->weight = $cartItem['shipping']['weight'];
						}
					}
				}
			}
...
add_filter( 'woocommerce_package_rates', 'modify_ups_shipping_method' , 10, 2 );
function modify_ups_shipping_method( $rates, $package ) {
	//$package['rates']['ups:12']->cost += 100;
	//$rates['ups:12']->cost += 100;
	//echo '<pre>';
...
function insert_product_shipping_code() { ?>
	<script>
		var estimateTimeout = null;
		
		function estimateProductShipping(height, width, weight, zipCode) {
			var estimateDebug = true;
			if (estimateDebug) {
				console.log('calculating shipping with the following');
				console.log('weight (lbs)', weight);
				console.log('height (inches)', height);
				console.log('width (inches)', width);
				console.log('zipCode', zipCode);
			}
			
            //If we get here, we've got the height and width, pass it via ajax.
			var estimateResultsBox = jQuery('#ship_estimate_amount');
			jQuery.post(
				...
				function(response){
					jQuery('#ship_estimate_loading').fadeOut('fast');
					var results = jQuery.parseJSON(response);
					if (estimateDebug) {
						console.log('Estimate: ',results);
					}
					if (results['status']) {
						var allRates = results['all_rates'];
						
						if (allRates == null) {
                            var rate = results['rate'];
							var rateAmount = '$' + rate['rate'];
							var rateLabel = rate['label'];
							var rateHtml = '<div class="ship_estimate_success">Estimate: '+rateLabel + ' / ' + rateAmount+'</div>';
                        } else {
						
							if (allRates.length > 1) {
								//Multiple rates...
								var rateHtml = '<div class="ship_estimate_success">Estimate:<table>';
								jQuery.each(allRates, function(rateIdx, rateItem) {
									var rateAmount = '$' + rateItem['cost'];
									var rateLabel = rateItem['label'];
									var rateSlug = rateItem['slug'];
									rateHtml += '<tr><td data-rateSlug="'+rateSlug+'">'+rateLabel+'</td><td>'+rateAmount+'</td></tr>';
								});
								rateHtml += '</table></div>';
								//rateHtml = '<div class="ship_estimate_success">Estimate: '+rateLabel + ' / ' + rateAmount+'</div>';
						...
						estimateResultsBox.html(rateHtml);
                    } else {
						var errorMsg = '<div class="ship_estimate_error">'+results['error']+'</div>';
						estimateResultsBox.html(errorMsg);
					}
				}
			);
        }
		
		function calculateProductDimensions(clickedButton) {
			if (estimateTimeout !== null) {
                clearTimeout(estimateTimeout);
            }
			
			if (clickedButton !== true) { clickedButton = false; }
			
            var gravityForm = jQuery('form[id^="gform_"]');
			var gravityFormId = parseInt(gravityForm.attr("id").replace('gform_',''));
			//Get Zip Code from User.
			var zipCode = gravityForm.find('#ship_estimate_zipcode').val();
			if (zipCode == "" || zipCode == undefined || zipCode == null) {
				if (clickedButton) {
                    gravityForm.find('#ship_estimate_zipcode').focus();
                }
				return false;
			}
			
			//Check for Height Fields.
			var heightFeetInput = gravityForm.find('.field-height-ft .ginput_container > input').val();
			var heightInchInput = gravityForm.find('.field-height-in .ginput_container > input').val();
			var height = parseFloat(heightFeetInput * 12) + parseFloat(heightInchInput);
			
			var widthFeetInput = gravityForm.find('.field-width-ft .ginput_container > input').val();
			var widthInchInput = gravityForm.find('.field-width-in .ginput_container > input').val();
			var width = parseFloat(widthFeetInput * 12) + parseFloat(widthInchInput);
			
			if (height <= 0 || width <= 0) {
				estimateTimeout = null;
				return;
			}
			var numSides = parseInt(gravityForm.find('.num-sides').find('select').val());
			if (numSides !== 1 && numSides !== 2) { numSides = 1; }
			var weight = (numSides * (heightFeetInput +(heightInchInput/12)) * (widthFeetInput+(widthInchInput/12)) * 1.5);
			weight = weight / 100;	//Without this, the formula returns back like 300, or 600 lbs...
			
			width = 4;	//same as length (since it'll be rolled up...). Need to calculate this as well.
			
			estimateTimeout = setTimeout(function() {
				//with height and width, pass it along to estimateProductShipping...
				jQuery('#ship_estimate_loading').fadeIn('fast');
				jQuery('#ship_estimate_amount').html('');
				estimateProductShipping(height, width, weight, zipCode);
			}, 500);	//1 second timeout to prevent unneed ajax calls.
        }
		
		if (window.jQuery) {
			(function($) {
				//Check for gravity form items...
				var isSingleProduct = (($('body.single-product').length >= 1) ? true:false);
				if (isSingleProduct) {
					setTimeout(function() {
						var gravityForm = $('form[id^="gform_"]');
						var hasGravityForm = ((gravityForm.length >= 1) ? true:false);
						if (hasGravityForm) {
							//We need to add the html markup to the gravity form.
							var zipMarkup = '<li class="ship_estimate_wrapper" style="display: none;">';
								zipMarkup += '<h3>Estimate Shipping</h3>';
								zipMarkup += '<label for="ship_estimate_zipcode">Enter your Zip Code</label>';
								zipMarkup += '<div>';
								zipMarkup += '<input type="text" id="ship_estimate_zipcode" style="width: 100%; max-width: 100px; display: inline-block; vertical-align: middle;" maxlength="5">';
								zipMarkup += '<button style="float: none; margin-left: 10px; padding: 10px 0;" id="ship_estimate_button" class="button alt gform_button">Estimate</button>';
								zipMarkup += '<span id="ship_estimate_loading"><img src="'+gf_global.spinnerUrl+'"></span>';
								zipMarkup += '</div>';
								zipMarkup += '<div id="ship_estimate_amount"></div>';
								zipMarkup += '</li>';
							
							var productTotalsContainer = gravityForm.find('.product_totals > ul');
							if (productTotalsContainer.length <= 0) { return; }
							
							productTotalsContainer.append(zipMarkup);
							
							$('.ship_estimate_wrapper').slideDown('fast');	//Show it.
							
                            var gravityFormId = parseInt(gravityForm.attr("id").replace('gform_',''));
							//Check for Height Fields.
							var heightFeetInput = gravityForm.find('.field-height-ft .ginput_container > input');
							var heightInchInput = gravityForm.find('.field-height-in .ginput_container > input');
							var hasHeight = (heightFeetInput.length && heightInchInput.length) ? true : false;
							
							var widthFeetInput = gravityForm.find('.field-width-ft .ginput_container > input');
							var widthInchInput = gravityForm.find('.field-width-in .ginput_container > input');
							var hasWidth = (widthFeetInput.length && widthInchInput.length) ? true : false;
							
							if (hasHeight && hasWidth) {
                                //We have height and width items objects. We need to bind some events to the change event of them all.
								heightFeetInput.on('change', function() { calculateProductDimensions(false); });
								heightInchInput.on('change', function() { calculateProductDimensions(false); });
								widthFeetInput.on('change', function() { calculateProductDimensions(false); });
								widthInchInput.on('change', function() { calculateProductDimensions(false); });
								
								$('#ship_estimate_button').on('click', function(e) {
									e.preventDefault();
									calculateProductDimensions(true);
								});
                            }
                        }
					}, 500);	//Moves this to the end of the stack, AFTER gravity forms loads.
                }
			})( jQuery );
		}
	</script>
...
add_action( 'wp_footer', 'insert_product_shipping_code', 20, 0 );
...
function estimate_shipping($zipCode=null, $weight=null, $dimensions=null) {
	$return = array(
        'status'=>false,
        'message'=>'DEFAULT',
        'debug'=>"",
	...
	require_once($pluginPath."woocommerce-shipping-ups/includes/class-wc-shipping-ups.php");

	$hasValidAttributes = true;
	$attrs = array();
	
	$attrs['weight'] = intval($weight);
	$attrs['height'] = intval($dimensions['height']);
	$attrs['width'] = intval($dimensions['width']);
	$attrs['length'] = intval($dimensions['length']);
	
	$variation_data = $cart_item_data = array();
	$cart_item_key = "irrelephant";	//Clearly doesn't pertain to elephants, therefore irrelephant. /endjoke.
	
	$quantity = 1;
	//$product_data = array();
	$product_data = new shippingDecorator(new stdClass);
	$product_data->length = $attrs['length'];
	$product_data->height = $attrs['height'];
	$product_data->width = $attrs['width'];
	$product_data->weight = $attrs['weight'];

	$cartItem = apply_filters( 'woocommerce_add_cart_item', array_merge( $cart_item_data, array(
		'product_id'    => '',
		'variation_id'  => '',
	...
	$package = array (
		...
		'contents_cost' => 0,             //Not Needed
		'applied_coupons' => array(),     //Not Needed
		'user' => array ( 'ID' => 1 ),
		'destination' => array (
			'country' => 'US',
			'state' => "",
	...
	$shipper = new WC_Shipping_UPS();
	$shipper->calculate_shipping($package);
	
	
	if (is_array($shipper->rates)) {
		$rates = $shipper->rates;
		$lowestPrice = 9999999;
		$lowestRateLabel = "DEFAULT";
		foreach($rates as $rate) {
			$rateLabel = $rate->label;
			$rateAmount = $rate->cost;
			if ($rateAmount < $lowestPrice) {
				$lowestPrice = $rateAmount;
				$lowestRateLabel = $rateLabel;
			}
		}
		
		if ($lowestPrice == 9999999) {
			$return['message'] = "Unable to get rates.";
		} else {
			$return['status'] = true;
			$return['message'] = array(
				"rate"=>$lowestPrice,
				"label"=>$lowestRateLabel
			);
			$return['rates'] = $shipper->rates;
		}
	} else {
		$return['message'] = "Unable to get rates.";
	}
...
add_action( 'wp_ajax_estimate_shipping', 'ajax_estimate_shipping' );
add_action( 'wp_ajax_nopriv_estimate_shipping', 'ajax_estimate_shipping' );

function sortByOrder($a, $b) {
    return $a->cost - $b->cost;
}

function ajax_estimate_shipping() {
	...
	if ($_POST['action'] == 'estimate_shipping') {
		$width = intval($_POST['width']);
		$height = intval($_POST['height']);
		$weight = intval(ceil($_POST['weight']));	//Move to the highest value in lbs...
		$zipCode = intval($_POST['zipcode']);
		
		if ($width > 0 && $height > 0 && !empty($zipCode) && $weight > 0) {
			//If both are > 0, lets generate a fake cart object and test shipping it...
			$length = 4;	//4 inches? Not sure how to calculate it...
			$dimensions = array(
				'height' => $height,
				'width' => $width,
				'length' => $length,
			);
			$estimateResults = estimate_shipping($zipCode, $weight, $dimensions);
			$removeRateSlugs = array(
				'next-day-air-early-am-ups',
				'next-day-air-ups',
			);
			if ($estimateResults['status'] === true) {
				$allRates = $estimateResults['rates'];
				foreach($allRates as $rateKey=>$rate) {
					//print_r($rate);
					$rateSlug = sanitize_title($rate->label);
					if (in_array($rateSlug, $removeRateSlugs)) {
						unset($allRates[$rateKey]);
						continue;
					}
					$allRates[$rateKey]->slug = $rateSlug;
				}
				//Remove certain rates...
				
				usort($allRates, 'sortByOrder');
				
				$results['rate'] = $estimateResults['message'];
				$results['status'] = true;
				$results['error'] = false;
				$results['all_rates'] = $allRates;
			} else {
				$results['error'] = $estimateResults['message'];
			}
			
			//$results['debug'] = $estimateResults;
		} else {
			//If either are 0, we can't estimate... return false...
			$results['error'] = 'Unable to get estimate. [DATA]';
		}
	} else{
		$results['error'] = 'Unable to get estimate [ACTION]';
	}
